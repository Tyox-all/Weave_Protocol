#!/usr/bin/env node
/**
 * weave-claude-code CLI
 *
 *   weave-claude-code init [--matcher=X] [--fail-closed]   Install the WARD hook
 *   weave-claude-code disable                              Remove the WARD hook
 *   weave-claude-code status                               Show current config + WARD
 *   weave-claude-code test <tool> [--input=JSON]           Dry-run a tool call
 *   weave-claude-code hook <event> [--fail-closed]         Hook handler (called by CC)
 *   weave-claude-code help
 */

import { existsSync, readFileSync } from 'node:fs';
import { resolve } from 'node:path';
import { parseWard } from '@weave_protocol/ward';
import {
  installHook,
  removeHook,
  readSettings,
...
function banner(): void {
  console.error(`
${c.cyan}${c.bold}🛡️  weave-claude-code${c.reset}
${c.gray}WARD.md enforcement for Claude Code via hooks${c.reset}
`);
}
...
function help(): void {
  banner();
  console.error(`${c.bold}Usage:${c.reset} weave-claude-code <command> [options]\n`);
  ...
  console.error(`  ${c.cyan}hook${c.reset} ${c.gray}<event>${c.reset}                       Internal: called by Claude Code as a hook`);
  console.error(`  ${c.cyan}help${c.reset}                               Show this message\n`);
  console.error(`${c.bold}Examples:${c.reset}`);
  console.error(`  ${c.gray}$${c.reset} weave-claude-code init`);
  console.error(`  ${c.gray}$${c.reset} weave-claude-code status`);
  console.error(`  ${c.gray}$${c.reset} weave-claude-code test Bash --input='{"command":"rm -rf ~/.ssh"}'`);
  console.error(`  ${c.gray}$${c.reset} weave-claude-code disable\n`);
  console.error(`${c.gray}Docs: https://github.com/Tyox-all/Weave_Protocol/tree/main/adapter-claudecode${c.reset}\n`);
}
...
function runInit(args: string[]): number {
  ...
  console.error(`${c.green}✓${c.reset} ${verb} WARD hook in ${c.cyan}${claudeSettingsPath()}${c.reset}`);
  ...
  console.error('');
  console.error(`Next:`);
  console.error(`  ${c.gray}1.${c.reset} Drop a ${c.cyan}WARD.md${c.reset} in your project root (or at ${c.cyan}~/.claude/WARD.md${c.reset} for a user-global policy)`);
  console.error(`  ${c.gray}2.${c.reset} Open Claude Code in that project — the hook will gate tool calls automatically`);
  console.error(`  ${c.gray}3.${c.reset} Run ${c.cyan}weave-claude-code status${c.reset} to verify`);
  console.error('');
  return 0;
}
...
function runDisable(): number {
  ...
  if (result.removed === 0) {
    console.error(`${c.yellow}⚠${c.reset}  No WARD hooks found in ${claudeSettingsPath()}.`);
    return 0;
  }
  console.error(`${c.green}✓${c.reset} Removed ${result.removed} WARD hook entry from ${c.cyan}${claudeSettingsPath()}${c.reset}`);
...
function runStatus(): number {
  ...
  console.error(`${c.bold}Claude Code config${c.reset}`);
  console.error(`${c.gray}─${c.reset}`.repeat(60));
  console.error(`  ${c.gray}Config dir:${c.reset}     ${c.cyan}${claudeConfigDir()}${c.reset}`);
...
function runTest(args: string[]): number {
  const tool = args[0];
  if (!tool) {
    console.error(`${c.red}✗${c.reset} Usage: weave-claude-code test <tool> [--input='{"file_path":"..."}']`);
    return 2;
  }
    ...
    switch (cmd) {
      ...
      case 'hook': {
        ...
        if (event === 'pre-tool-use' || !event) {
          ...
          // runPreToolUseHook calls process.exit, but TS doesn't know
          return;
        }
        // Other hook events: silently allow for now (forward compat)
